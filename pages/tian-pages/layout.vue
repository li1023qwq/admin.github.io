		...
		<view class="layout-container" :style="containerStyle">
			...
			<view class="preview">
				...
				<view class="items" :style="containerStyle">
					<scroll-view scroll-y="true" style="height: 1800rpx;">
						<tian-pages :configs="pageConfigs" :preview="true" @click="editItem" @deletes="delItem"
							@sort="sortItem">
						</tian-pages>
					</scroll-view>
				</view>
					...
					<scroll-view scroll-y="true">
						<uni-forms ref="form" :value="formData" validateTrigger="bind">
							<!-- <uni-forms-item name="title" label="标题" labelWidth="90">
								<uni-easyinput v-model="formData.title" @input="testInput" />
							</uni-forms-item>
							...
							<!-- 读取原来的数据 -->
							<view v-if="formData.items && formData.data">
								...
								<view v-for="(item2,index2) in formData.items" :key="index2"
									...
									<uni-forms ref="form">
										<uni-forms-item v-for="(item,index3) in formData.data" :name="index3"
								...
								<!-- <view class="padding-top-12 font-38">
									<text>新增数据</text>
								</view>
								<view class="margin-right-20">
									<uni-forms ref="form" validateTrigger="bind">
										<uni-forms-item v-for="(item,index) in formData.component.data" :name="index"
											:key="index" :label="item.title" labelWidth="90">
											<block v-if="item.enum">
												<uni-data-checkbox v-if="item.enum.collection" v-model="item.value"
													:collection="item.enum.collection" :field="item.enum.field" />
												<uni-data-checkbox v-else v-model="item.value" :localdata="item.enum" />
											...
											<switch v-else-if="item.bsonType=='bool'"
												@change="binddata('state', $event.detail.value)"
												:checked="!!item.value" />
											<cloud-image v-else-if="item.bsonType=='image'" :name="index"
												v-model="item.value" />
										</uni-forms-item>
									</uni-forms>
									<view class="ti-flex">
										...
										<button type="primary" class="uni-button" style="width: 150px;"
											@click="saveData">新增数据</button>
									</view>
								</view> -->
							</view>
						</uni-forms>
	...
	export default {
		...
		methods: {
			...
			add(item) {
				...
				this.initDefaultAttr(newData, newData)
				if (index != -1) {
					index += 1;
			...
			editItem(item) {
				this.edit(this.dataList.filter(e => e._id == item._id)[0])
			},
			edit(item) {
				console.log("edit", item.title, item.component_id)
				this.formData = {};
				this.attrCurrent = 0;
				this.dataList.forEach(i => {
					i.checked = i._id == item._id;
				})
				console.log("edit this.dataList", this.dataList)
				// item.checked = true
				//处理属性默认值
			...
			testInput(e) {
				...
				console.log("updateData", JSON.stringify(this.dataList.map(e => `${e.title} ${e.component_id}`)))
			},
			...
			updateData(item) {
				...
				console.log("updateData", item, JSON.stringify(this.dataList.map(e => `${e.title} ${e.component_id}`)))
			},
			...
			sortItem(item, index, type) {
				//排序
				console.log("item排序", item, index, type)
				if (type == "up") {
					//向上移动
					let tmp = this.dataList[index - 1];
					this.dataList[index - 1] = this.dataList[index]
			...
			initDefaultAttr(component, item) {
				...
				component.attributes.state = {
					bsonType: "int",
					defaultValue: item.state ? 1 : 0,
					title: "是否显示",
					enum: this.formOptions.state_localdata,
					value: item.state ? 1 : 0,
				}
			},
			loadAssembly() {
				db.collection("tian-page-components").where("is_page!=true").orderBy("posid asc").limit(100).get().then(({
					result
				}) => {
