...
export default {
	data() {
		...
		Object.keys(mpPlatform).concat(['icon_url', 'quickapp']).forEach(key => data.middleware_img[`${key}`] = {})
		return data
	},
	methods: {
		...
		formatFormData() {
			this.setFormData('screenshot', this.screenshotList.map(item => item.url))

			/* this.keepItems = this.platFormKeys.concat(['icon_url', 'screenshot', 'status'])
			this.platFormKeys.forEach(key => {
				if (!this.getPlatformChcekbox(key)) {
					this.setFormData(key, {})
				}
			}) */
			this.keepItems = this.platFormKeys
				.filter(key => this.getPlatformChcekbox(key))
				.concat(['icon_url', 'screenshot', 'status'])

			if (this.formData.h5 && this.formData.h5.url)
				this.keepItems.push('h5');

			if (this.formData.quickapp && (this.formData.quickapp.name || this.formData.quickapp.qrcode_url))
				this.keepItems.push('quickapp');
		},
		getPlatformChcekbox(mp_name) {
			const checkbox = this.$refs[`${mp_name}_checkbox`]
			return checkbox && checkbox[0] && checkbox[0].checkboxChecked
		},
		setPlatformChcekbox(mp_name, value = false) {
			const checkbox = this.$refs[`${mp_name}_checkbox`]
			checkbox && checkbox[0] && (checkbox[0].checkboxChecked = value)
		},
		selectFile() {
			if (this.hasPackage) {
				uni.showToast({
